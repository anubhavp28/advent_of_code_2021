# ---------- Part 1 ------------- #
# Do we need the structure of the transmission to get the sum total of all version numbers?
# NO!

INPUT = ""

# convert hex to binary
def hex_to_bin(hex_input):
    out = bin(int(hex_input, 16))[2:]
    # python kills leading zero on converting from hex to bin
    rem = len(out) % 4
    rem = (4 - rem) % 4
    out = '0' * rem + out
    return out

def bin_to_dec(bin_input):
    return int(bin_input, 2)
    
input = hex_to_bin(INPUT)
input = input.rstrip("0")

def consume_input(size):
    global input
    consumed = input[:size]
    input = input[size:]
    return consumed

ans = 0    
while len(input) > 0:
    # start of a packet
    packet_version = bin_to_dec(consume_input(3))
    packet_type = bin_to_dec(consume_input(3))
    ans += packet_version
    
    if packet_type == 4:
        # literal packet
        while True:
            five_bit_group = consume_input(5)
            if five_bit_group[0] == '0':
                break
    else:
        # operator packet
        length_type_id = consume_input(1)
        if length_type_id == '0':
            consume_input(15)
        else:
            consume_input(11)

print(ans)